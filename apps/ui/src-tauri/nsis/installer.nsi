...
Section Install
  ...
  ; ═══ XS20 CUSTOM: detener el servicio para poder sobreescribir xs20-service.exe ═══
  ; En un upgrade, nssm.exe ya esta en $INSTDIR. El sc.exe cubre el fallback
  ; (instalacion NSSM manual vieja en otra carpeta) y el taskkill remata un
  ; proceso suelto (modo app-hijo). Sin esto, el .exe lockeado rompe la copia.
  nsExec::ExecToLog '"$INSTDIR\nssm.exe" stop WienerXS20Service'
  Pop $R0
  nsExec::ExecToLog '"$SYSDIR\sc.exe" stop WienerXS20Service'
  Pop $R0
  Sleep 3000
  ...
  ; ═══ XS20 CUSTOM: registrar y arrancar el servicio de Windows ═══
  ; install-service.ps1 es idempotente: instala o re-apunta el servicio,
  ; re-aplica la config completa, crea la regla de firewall y lo arranca.
  DetailPrint "Instalando el servicio WienerXS20Service..."
  nsExec::ExecToLog 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$INSTDIR\install-service.ps1" -InstallDir "$INSTDIR"'
  Pop $R0
  ${If} $R0 != 0
    MessageBox MB_OK|MB_ICONEXCLAMATION "No se pudo registrar el servicio de Windows (codigo $R0). La app funciona igual, pero solo recibe resultados mientras este abierta. Ver C:\ProgramData\WienerXS20\logs\install-service.log"
  ${EndIf}
  ; ═══ FIN XS20 CUSTOM ═══
SectionEnd
...
Section Uninstall
  ...
  ; ═══ XS20 CUSTOM: detener y desregistrar el servicio de Windows ═══
  ; No borra C:\ProgramData\WienerXS20 (la DB con resultados se preserva).
  nsExec::ExecToLog 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$INSTDIR\uninstall-service.ps1" -InstallDir "$INSTDIR"'
  Pop $R0
  ; ═══ FIN XS20 CUSTOM ═══
